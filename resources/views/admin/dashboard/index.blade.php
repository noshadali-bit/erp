 ...
 <div class="container-fluid">
            ...
            <div class="media media-wrapper justify-content-between">
              <div class="media-body">
                <div class="greeting-user">
                  <h4 class="f-w-600">Welcome to Performance Dashboard</h4>
                  <p>Here You Will see All Deaprtments Performance</p>
    ...
    @if($user->role_id == 2)
        @if($user->department->parent_id == null && $user->department->sub_departments->count() > 0)
             @foreach ($user->department->sub_departments as $departm)
         <div class="col-xxl-4 col-xl-7 col-md-6 col-sm-5 box-col-6">
            <div class="card height-equal">
              <div class="card-header card-no-border">
                <div class="header-top">
                  <h5>{{($departm->manager) ? $departm->manager->name.' - ' : ''}} <span>{{$departm->name}}</span></h5>
              ...
              <div class="card-body pt-0">
                <div class="row recent-wrapper">
                  <div class="col-xl-6">
                    <div class="recent-chart" data="{{ $departm->performance($month)}}">
                    ...
                    <ul class="order-content">
                      <li> <span class="recent-circle bg-primary"> </span>
                        <div> <span class="f-light f-w-500">Total Task</span>
                          <h4 class="mt-1 mb-0">{{ ($departm->target($month) != 0) ? $departm->target($month) : $departm->allTasks($month) }}<span class="f-light f-14 f-w-400 ms-1">({{ Carbon\Carbon::createFromFormat('Y-n', $month)->format('F Y');}}) </span></h4>
                        </div>
                      </li>
                      <li> <span class="recent-circle bg-info"></span>
                        <div> <span class="f-light f-w-500">Complete Task</span>
                          <h4 class="mt-1 mb-0">{{$departm->achiveTarget($month)}}</h4>
                        </div>
                      </li>
                      <li>
                        <div class="see_team">
                          <a href="{{route('department-detail', $departm->slug )}}" class="btn btn-primary">Team Details</a>
                        </div>
                      </li>
                    </ul>
                  </div>
    ...
    @else
      ...
      <div class="col-xxl-4 col-xl-7 col-md-6 col-sm-5 box-col-6">
        <div class="card height-equal">
          <div class="card-header card-no-border">
            <div class="header-top">
              <h5>{{($department->manager) ? $department->manager->name : ''}}  <span>( {{$department->name}} )</span></h5>
            ...
            <div class="row recent-wrapper">
              ...
              <div class="col-xl-6">
                <ul class="order-content">
                  <li> <span class="recent-circle bg-primary"> </span>
                    <div> <span class="f-light f-w-500">Total Task</span>
                      <h4 class="mt-1 mb-0">{{ ($department->target($month) != 0) ? $department->target($month) : $department->allTasks($month) }}<span class="f-light f-14 f-w-400 ms-1">({{ Carbon\Carbon::createFromFormat('Y-n', $month)->format('F Y');}}) </span></h4>
                    </div>
                  </li>
                  <li> <span class="recent-circle bg-info"></span>
                    <div> <span class="f-light f-w-500">Complete Task</span>
                      <h4 class="mt-1 mb-0">{{$department->achiveTarget($month)}}</h4>
                    </div>
                  </li>
                  <li>
                    <div class="see_team">
                      <a href="{{route('department-detail', $department->slug )}}" class="btn btn-primary">Team Details</a>
                    </div>
                  </li>
                </ul>
              </div>
