...
class user
{
    ...
    {
        Assert::integer($id, 'The user ID must be an integer. Got: %s');
        Assert::greaterThan($id, 0, 'The user ID must be a positive integer. Got: %s');
    }
}
```

If you create an user with an invalid ID, an exception is thrown:

```php
new user('foobar');
// => Webmozart\Assert\InvalidArgumentException:
//    The user ID must be an integer. Got: string

new user(-10);
// => Webmozart\Assert\InvalidArgumentException:
//    The user ID must be a positive integer. Got: -10
```
...
```php
Assert::allIsInstanceOf($users, 'Acme\user');
```
